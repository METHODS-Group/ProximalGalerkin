...
if __name__ == "__main__":
    ...
    polynomial_order = 1
    u_lvpp, max_it, num_newton_steps = solve_problem(
        args.infile,
        polynomial_order,
        maximum_number_of_outer_loop_iterations=max_iter,
        alpha_scheme="geometric",
        alpha_max=1e3,
        tol_exit=tol,
    )

    mesh = u_lvpp.function_space.mesh
    degree = max(polynomial_order, mesh.geometry.cmap.degree)
    V_out = dolfinx.fem.functionspace(mesh, ("Lagrange", degree))
    ...
    u_lvpp_2, max_it_2, num_newton_steps2 = solve_problem(
        args.infile,
        polynomial_order + 1,
        maximum_number_of_outer_loop_iterations=max_iter,
        alpha_scheme="geometric",
    ...
    print(
        f"{_GREEN}{args.infile} llvp iterations: (P={polynomial_order}) {max_it} {num_newton_steps} {sum(num_newton_steps)}{_color_reset}"
    )
    print(
        f"{_RED}{args.infile} llvp iterations: (P={polynomial_order + 1}) {max_it_2} {num_newton_steps2} {sum(num_newton_steps2)}{_color_reset}"
    )
    print(f"{args.infile} Ipopt iterations: (With hessian) {ipopt_iteration_count[True]}")
    print(
        f"{_BLUE}{args.infile} Ipopt iterations: (Without hessian) {ipopt_iteration_count[False]}{_color_reset}"
    )
    print(f"{_GREEN}{args.infile} SNES iterations: {num_snes_iterations}{_color_reset}")

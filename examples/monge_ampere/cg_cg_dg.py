...
for mesh in mh:
    ...
    if nonsmooth:
        u_exact = conditional(le(dot(X, X), 1/4), 2*dot(X, X), 2*dot(X, X) + 2*(sqrt(dot(X, X)) - 1/2)**2)
    else:
        u_exact = exp(dot(X, X)/2)

    def hessian(u):
        return as_tensor([[u.dx(0).dx(0), u.dx(0).dx(1)],
                          [u.dx(1).dx(0), u.dx(1).dx(1)]])

...
def expm3(A):
    ...
    val_neg = exp(x1_neg) * dot(A - x2_neg*I, A - x3_neg*I) / ((x1_neg - x2_neg)*(x1_neg - x3_neg)) + \
              ...
              exp(x3_neg) * dot(A - x1_neg*I, A - x2_neg*I) / ((x3_neg - x1_neg)*(x3_neg - x2_neg))

    # Case 2: Delta zero, either triple root or two roots equal one different
    #case_zero = eq(Delta, 0)
    case_zero = lt(Delta, 1e-14)
    x1_zero = conditional(eq(a, 0), 0, conditional(lt(b, 0), +2*(-b/2)**(1/3), -2*(b/2)**(1/3))) - p/3
    x2_zero = conditional(eq(a, 0), 0, conditional(lt(b, 0), -1*(-b/2)**(1/3), +1*(b/2)**(1/3))) - p/3
    x3_zero = conditional(eq(a, 0), 0, conditional(lt(b, 0), -1*(-b/2)**(1/3), +1*(b/2)**(1/3))) - p/3

    # if a == 0: triple root
    val_zero_one = exp(x1_zero) * (1/2 * dot(A - x1_zero*I, A - x1_zero*I) + (A - x1_zero*I) + I)
    ...
    coeffAsq = (c*exp(lam) - c*exp(r)*cos(c) - lam*exp(r)*sin(c) + r*exp(r)*sin(c))/(c*(c**2 + lam**2 - 2*lam*r + r**2))
    coeffA   = (c**2*exp(r)*sin(c) - 2*c*r*exp(lam) + 2*c*r*exp(r)*cos(c) + lam**2*exp(r)*sin(c) - r**2*exp(r)*sin(c))/(c*(c**2 + lam**2 - 2*lam*r + r**2))
    coeffI   = (c**3*exp(lam) - c**2*lam*exp(r)*sin(c) + c*lam**2*exp(r)*cos(c) - 2*c*lam*r*exp(r)*cos(c) + c*r**2*exp(lam) - lam**2*r*exp(r)*sin(c) + lam*r**2*exp(r)*sin(c))/(c*(c**2 + lam**2 - 2*lam*r + r**2))

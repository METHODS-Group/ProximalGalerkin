...
def expm2(A):
    ...
    # Branch 1: e == 0
    cond1 = ufl.eq(e, 0)
    branch1 = ufl.exp((a + d) / 2) * ufl.as_tensor([[1 + (a - d) / 2, b], [c, 1 - (a - d) / 2]])

    # Branch 2: e > 0
    cond2 = ufl.gt(e, 0)
    delta = (1 / 2) * ufl.sqrt(e)
    branch2 = ufl.exp((a + d) / 2) * ufl.as_tensor(
        [
            [ufl.cosh(delta) + (a - d) / 2 * ufl.sinh(delta) / delta, b * ufl.sinh(delta) / delta],
    ...
    # cond3 = ufl.lt(e, 0)
    eps = (1 / 2) * ufl.sqrt(-e)
    branch3 = ufl.exp((a + d) / 2) * ufl.as_tensor(
        [
            [ufl.cos(eps) + (a - d) / 2 * ufl.sin(eps) / eps, b * ufl.sin(eps) / eps],
            [c * ufl.sin(eps) / eps, ufl.cos(eps) - (a - d) / 2 * ufl.sin(eps) / eps],
        ]
    )

    return ufl.conditional(cond1, branch1, ufl.conditional(cond2, branch2, branch3))
...
if __name__ == "__main__":
    ...
    for tensor in tensors:
        ...
        tensor_expr = dolfinx.fem.Expression(
            t_ufl, V.element.interpolation_points(), comm=domain.comm
        )
        ...
        np.testing.assert_allclose(
            scipy.linalg.expm(A.x.array.reshape(*value_shape)), expA.x.array.reshape(*value_shape)
        )

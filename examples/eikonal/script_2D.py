...
def solve_problem(
    ...
    el_1 = basix.ufl.element("RT", mesh.topology.cell_name(), degree + 1)
    ...
    bp_u = dolfinx.io.VTXWriter(
        mesh.comm,
        dir / f"u_{N}_{M}_{L}_{H}_{degree}_{quadrature_degree}_{mesh.basix_cell()}.bp",
        [u_out],
        engine="BP4",
    )
...
if __name__ == "__main__":
    ...
    for j, degree in enumerate(degrees):
        for i, N in enumerate(Ns):
            hs[j, i], L2_errors[j, i], H10_errors[j, i], total_iterations[j, i] = solve_problem(
                N, N, L=L, H=H, degree=degree
            )
    ...
    def format_tex(float_number):
        ...
        return r"${0}\cdot 10^{{{1}}}$".format(mantissa_format, str(int(exponent)))
    ...
    for i, degree in enumerate(degrees):
        ...
        print(
            df.to_latex(
                ...
                caption=f"Convergence rates for the eikonal equation with DG-{degree} elements",
                label=f"tab:eikonal_{degree}",
                index=False,
            )
        )

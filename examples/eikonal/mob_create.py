...
def create_mobius_mesh(M, degree=3):
    ...
    for i in range(M):
        ...
        # Edge 2
        for j in range(1, degree):
            cell_vertices[i, 4 + 2 * (degree - 1) + j - 1] = (
                base * i + (j + 1) * (degree + 1) - 1
            )

        # Edge 3
        for j in range(1, degree):
            cell_vertices[i, 4 + 3 * (degree - 1) + j - 1] = (
                base * i + degree * (degree + 1) + j
            )
        ...
        for j in range(1, degree):
            for k in range(1, degree):
                cell_vertices[
                    i, 4 + 4 * (degree - 1) + (j - 1) * (degree - 1) + k - 1
                ] = base * i + j * (degree + 1) + k
    ...
    for j in range(1, degree):
        cell_vertices[M - 1, 4 + 3 * (degree - 1) + j - 1] = cell_vertices[
            0, 4 + (degree + 1) - j
        ]
